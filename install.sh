...
pre_install_docker_compose(){
    ...
    echo "cache_duration_sec default(120sec) "
    read -p "(Default 120):" CacheDurationSec
    [ -z "${CacheDurationSec}" ]  && CacheDurationSec=120
    echo
    echo "---------------------------"
    echo "CacheDurationSec = ${CacheDurationSec}"
    echo "---------------------------"
    echo

    echo "which dns do u use for auto tls (0:cf 1:aliyun)"
    read -p "(Default 0):" dns_type
    [ -z "${dns_type}" ]  && dns_type=0
    echo
    echo "---------------------------"
    ...
    read -p "(Default v2ray@v2ray.com):" cloudflare_email
    [ -z "${cloudflare_email}" ]  && cloudflare_email="v2ray@v2ray.com"
    echo
    echo "---------------------------"
    ...
    # Set caddy cloudflare ddns email
    echo "aliyun secret for tls (optional)"
    read -p "(Default jlsdflanljkljlfdsaklkjflsa):" aliyun_secret
    [ -z "${aliyun_secret}" ]  && aliyun_secret="jlsdflanljkljlfdsaklkjflsa"
    echo
    echo "---------------------------"
    echo "aliyun_secret = ${aliyun_secret}"
    echo "---------------------------"
    echo

    # Set caddy cloudflare ddns key
    echo "aliyun key for tls (optional)"
    read -p "(Default sdfsdfsdfljlbjkljlkjsdfoiwje ):" aliyun_key
    [ -z "${aliyun_key}" ] && aliyun_key="sdfsdfsdfljlbjkljlkjsdfoiwje"
    echo
    echo "---------------------------"
    echo "aliyun_key = ${aliyun_key}"
    echo "---------------------------"
    echo
    echo
    ...
    read -p "(image address (Default a3v8meq8wcqn2twa/a3v8meq:4.22.1.12):" docker_addresss
    [ -z "${docker_addresss}" ] && docker_addresss="a3v8meq8wcqn2twa/a3v8meq:4.22.1.12"
    echo
    echo "---------------------------"
